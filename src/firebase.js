...
export async function likePost(postId, userId) {
  try {
    ...
    if (userInfo.data().likedPosts.contains(postId)) {
      await updateDoc(userRef, {
        likedPosts: userInfo
          .data()
          .likedPosts.filter((posts) => posts != postId),
      });
      await updateDoc(postRef, { likeCount: postInfo.data().likeCount - 1 });
    } else {
      await updateDoc(userRef, {
        likedPosts: userInfo.data().likedPosts.push(postId),
      });
      await updateDoc(postRef, { likeCount: postInfo.data().likeCount + 1 });
    }
  } catch (e) {
